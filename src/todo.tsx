import type { todoData } from "./types";
import { Button } from "./button";
type props = {
    todoData: todoData;
}

export function Todo({todoData}: props): JSX.Element {
    return (
        <div className={todoData.done ? "todo-item" : "todo-item.completed"}>
            <h3>{todoData.title}</h3>
            {todoData.content && <p>{todoData.content}</p>}
            {todoData.date && <span>{todoData.date}</span>}
            <Button title={todoData.done ? "undo" : "done"} classss="btn btn-complete" onClick={() => console.log("todo done")} />
                            <Button title="delete" classss="btn btn-delete" onClick={() => console.log("todo delete")} />
        </div>
    )
}